...
for i in range(1, 4):
    ...
    if response.status_code == 200:
        ...
        recenttitles = soup.select('span.result-title')
        origintitles = soup.select('span.title')
        for title in recenttitles:
            title_text = list(title.stripped_strings)[0]
            print(title_text)
        for title in origintitles:
            title_text = list(title.stripped_strings)[0]
            print(title_text)
        print(len(recenttitles + origintitles))
    else: 
