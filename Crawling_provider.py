...
for i in range(1, 32): 
    ...
    if response.status_code == 200:
        ...
        for item in items:
            provider_span = item.select_one('.info-data .data')
            if provider_span:
                provider = provider_span.get_text(strip=True)
                data.append(provider)
            else:
                data.append('Unknown Provider')
    else: 
        print("Failed to fetch data on page", i)

df = pd.DataFrame(data, columns=['Provider'])

file_path = '/Users/kimdogyun/Downloads/data_results_provider.xlsx'
